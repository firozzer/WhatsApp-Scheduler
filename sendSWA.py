...
def sendScheduledMessage(name, message):
    try:
        ...
        try: #catches non connection to internet of WA
            ...
            sendImage('ss.png')
            driver.quit()
            return f"""Error: WA W could not reach your phone when trying to send "{message}" to "{name}"."""
        print('got searchbar')
...
# https://api.whatsapp.com/send?phone=971111111111&text=thisisatest direct API call but doesn't work unless you click on it inside web whatsapp as a chat
# https://web.whatsapp.com/send?phone=971111111111&text=thisisatest goes directly nice tho
